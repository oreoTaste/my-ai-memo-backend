...
export class UserService {
  ...
  async insertUser(insertUserDto: InsertUserDto):Promise<InsertUserResultDto> {
    try {
      ...
      let foundUser = await this.usersRepository.findOne({where: {loginId: insertUserDto.loginId}, select: ['id', 'isActive', 'name', 'createdAt']});
      if(foundUser) {
  ...
  async login(loginUserDto: LoginUserDto): Promise<LoginUserResultDto> {
    let user = (await this.usersRepository.findOne({where: loginUserDto, select: ['id', 'loginId', 'isActive', 'name', 'createdAt', 'adminYn']}));
    if(user) {
  ...
  async searchUser(searchUserDto: SearchUserDto): Promise<SearchUserResultDto> {
    ...
    let users = (await this.usersRepository.find({where: {loginId, name}, select: ['id', 'isActive', 'name', 'createdAt'], order: {id: "ASC"}, take: 100}));
    if(users.length == 0) {
  ...
  async retreiveUserBySeq(seq: number): Promise<User> {
    return await this.usersRepository.findOne({where: {id: seq}});
  }
  ...
  async listUser(idInsert: number): Promise<User[]> {
    return await this.usersRepository.find({order: {id: "ASC"}, take: 100});
  }
